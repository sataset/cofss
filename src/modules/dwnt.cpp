...
{
    double P; // |E_+|^2 + |E_-|^2
    int size = signal->right.size();
    for (int i = 0; i < size; ++i) {
        P = 1.0 - alpha_ns - alpha_0 / (1.0 + norm(signal->right[i] / P_sat));
        if (P < 0)
            P = 0.0;
        signal->right[i] *= sqrt(P);

        P = 1.0 - alpha_ns - alpha_0 / (1.0 + norm(signal->left[i]) / P_sat);
        if (P < 0)
            P = 0.0;
        signal->left[i] *= sqrt(P);
    }
